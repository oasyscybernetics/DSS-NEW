...
class DisplayFactory extends BaseFactory
{
     ...
     * @throws NotFoundException
     */
    public function getByLicence($licence,$macAddress = null)
    {
        if (empty($licence)) {
            throw new NotFoundException(__('Hardware key cannot be empty'));
        }

        $displays = $this->query(null, ['disableUserCheck' => 1,'license' => $licence, 'macAddress' => $macAddress]);
    ...
    public function query($sortOrder = null, $filterBy = [])
    {
        ...
        // if ($this->getSanitizer()->getString('macAddress', $filterBy) !== null) {
        //     $body .= ' AND display.macAddress = :macAddress ';
