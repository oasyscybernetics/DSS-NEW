...
class Soap
{
    ...
    protected function doRequiredFiles($serverKey, $hardwareKey, $httpDownloads)
    {
        ...
        try {
            ...
            foreach ($sth->fetchAll() as $row) {
                ...
                $media_sth = $dbh->prepare("SELECT mediaId, `name`, fileSize, media.type AS mediaType, storedAs, `requiredfile`.*FROM `media` INNER JOIN `requiredfile` ON `requiredfile`.itemId = `media`.mediaId WHERE `requiredfile`.displayId = ".$this->display->displayId." AND `requiredfile`.type = 'M' and `requiredfile`.complete = 0  and  `requiredfile`.itemId = ".$id." ORDER BY `name`");
                $media_sth->execute(array());
                $media_rows = $media_sth->rowCount();
                ...
                $file->setAttribute("mediaInventoryStatus", $overall_cnt);
        ...
        foreach ($layouts as $layoutId) {
            ...
            try {
                ...
                $layout_sth = $dbh->prepare("SELECT layoutId, layout, `requiredfile`.*  FROM `layout` INNER JOIN `requiredfile` ON `requiredfile`.itemId = `layout`.layoutId  WHERE `requiredfile`.displayId =". $this->display->displayId."  AND `requiredfile`.type = 'L'  and `requiredfile`.complete = 0 and `requiredfile`.itemId = ".$layoutId." ORDER BY layout");
                $layout_sth->execute(array());
                $layout_rows = $layout_sth->rowCount();
                ...
                $file->setAttribute("mediaInventoryStatus", $layout_cnt);
    ...
    protected function doSchedule($serverKey, $hardwareKey, $options = [])
    {
        ...
        try {
            ...
            $layout_sth = $dbh->prepare("SELECT layoutId, layout, `requiredfile`.*  FROM `layout` INNER JOIN `requiredfile` ON `requiredfile`.itemId = `layout`.layoutId  WHERE `requiredfile`.displayId =". $this->display->displayId."  AND `requiredfile`.type = 'L'  and `requiredfile`.complete = 0 ORDER BY layout");
            $layout_sth->execute(array());
            $layout_rows = $layout_sth->rowCount();

            $media_sth = $dbh->prepare("SELECT mediaId, `name`, fileSize, media.type AS mediaType, storedAs, `requiredfile`.*FROM `media` INNER JOIN `requiredfile` ON `requiredfile`.itemId = `media`.mediaId WHERE `requiredfile`.displayId = ".$this->display->displayId." AND `requiredfile`.type = 'M' and `requiredfile`.complete = 0  ORDER BY `name`");
            $media_sth->execute(array());
            $media_rows = $media_sth->rowCount();

            if($layout_rows > 0 || $media_rows > 0){
                $overall_cnt = 0;
            }else{
                $overall_cnt = 1;
            }
                ...
                foreach ($scheduleEvents as $scheduleEvent) {
                    ...
                    $is_priority = $this->getSanitizer()->int($row['isPriority']);
                     ...
                     if ($eventTypeId == Schedule::$LAYOUT_EVENT || $eventTypeId == Schedule::$INTERRUPT_EVENT || $eventTypeId == Schedule::$CAMPAIGN_EVENT) {
                        ...
                        $layout->setAttribute("mediaInventoryStatus", $overall_cnt);
        ...
        try {
            ...
            $default->setAttribute("mediaInventoryStatus", $overall_cnt);
    ...
    // setting File API
      ...
      protected function doSettingFiles($serverKey, $hardwareKey)
    {
        $this->logProcessor->setRoute('SettingFiles');

        // Sanitize
        $server_Key = $this->getSanitizer()->string($serverKey);
        ...
        $currentdate_time = strtotime($this->getDate()->getLocalDate());
