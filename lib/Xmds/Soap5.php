...
class Soap5 extends Soap4
{
    ...
    public function RegisterDisplay($serverKey, $hardwareKey, $displayName, $clientType, $clientVersion, $clientCode, $operatingSystem, $macAddress, $xmrChannel = null, $xmrPubKey = null)
    {
        ...
        try {
            $display = $this->displayFactory->getByLicence($hardwareKey,$macAddress);
        ...
        $display->firstAccessed = time();
        //$display->operatingSystem = $operatingSystem;
        $display->save(Display::$saveOptionsMinimum);


        // Check display History Data already Exists

        $current_date = date("Y-m-d");


        $sql_1 = "SELECT id,displayId,date(FROM_UNIXTIME(firstAccessed)) dt  FROM `display_history` WHERE license='$hardwareKey' and date(FROM_UNIXTIME(firstAccessed))='$current_date'";
        ...
        if($historyId > 0){
            ...
            // $update = $dbh->prepare('UPDATE `display_history` SET `lastAccessed` = :time_one WHERE `id` = :historyId');
            ...
            $insertSth = $dbh->prepare('
                    INSERT INTO display_history (displayId, display, firstAccessed, lastaccessed,license)
                        VALUES (:displayid, :display, :firstaccessed, :lastAccessed,:license)
                ');
