...
abstract class PlayerAction implements PlayerActionInterface
{
    ...
    public final function send($connection)
    {
        try {
            ...
            $context = null;
            $context = new \ZMQContext();
            $publisher = $context->getSocket(\ZMQ::SOCKET_XPUB);
            $publisher->bind($connection);
            //usleep(30000);           
            //ob_start();
            $count=0;
            //echo "Running..Serving test data on TCP port 5555 every 1 s \n <hr>";
            $reply = false;
            $n = 2;
            $channel = $this->channel;

            while (--$n >= 0)
            {  //doesn't work when this line is commented out
                
                $count++;
                $json=json_encode($message);
                $publisher->send($channel, \ZMQ::MODE_SNDMORE);
            ...
            if ($context instanceof \ZMQSocket && !$context->isPersistent()) {
                $context->disconnect($connection);
            }
            $publisher = null;
            $output = shell_exec('fuser -k 5555/tcp');
